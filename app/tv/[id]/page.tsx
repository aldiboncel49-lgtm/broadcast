...
import { fetchTmdbList, tmdbImage, FALLBACK_TV } from '@/lib/tmdb';
import { notFound } from 'next/navigation';

interface PageProps { params: { id: string } }

export function generateStaticParams() {
  return FALLBACK_TV.map((m) => ({ id: String(m.id) }));
}
...
export default async function DetailPage({ params }: PageProps) {
  ...
  const ytSearch = `https://www.youtube.com/results?search_query=${encodeURIComponent(`${title} ${year || ''} full series sub indo`)}`;

  return (
    <div>
      <Link href="/tv" className="font-mono text-xs phosphor-amber hover:text-phosphor-amberGlow flex items-center gap-1 mb-6">
        <ArrowLeft size={12} /> BACK TO CH 04
      </Link>
      <article className="grid md:grid-cols-[300px,1fr] gap-8">
        ...
        <div>
          ...
          {item.original_name && item.original_name !== title && (
            <div className="text-sm text-gray-500 font-mono mb-4">aka. {item.original_name}</div>
          )}
          ...
          <div className="mt-6 space-y-2">
            <a href={ytSearch} target="_blank" rel="noopener noreferrer" className="btn-broadcast flex items-center gap-2 w-fit">
              <ExternalLink size={14} /> SEARCH YOUTUBE (SUB INDO)
            </a>
            <div className="text-xs font-mono text-gray-500">No streams hosted on this site.</div>
          </div>
        </div>
      </article>
    </div>
